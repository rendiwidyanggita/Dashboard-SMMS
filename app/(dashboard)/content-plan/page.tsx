...
export default function ContentPlanPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const grouped = columnTitles.map((title) => ({
        ...
        items: data
          .filter(
            (item: any) => item.pillar?.toLowerCase() === title.toLowerCase(),
          )
          .map((item: any) => ({
            ...item,
            id: item.id_konten,
            ...
            time: item.tanggal_upload
              ? new Date(item.tanggal_upload).toLocaleTimeString("id-ID", {
                  hour: "2-digit",
                  minute: "2-digit",
                })
              : "-",
            status: item.status_konten
              ? item.status_konten.charAt(0).toUpperCase() +
                item.status_konten.slice(1)
              : "Pending",
  ...
  const handleCardClick = (item: ContentItem) => {
    ...
    setEditForm({
      ...
      format: item.format,
      date: item.tanggal_upload
        ? new Date(item.tanggal_upload).toISOString().split("T")[0]
        : "",
      time: item.tanggal_upload
        ? new Date(item.tanggal_upload)
            .toTimeString()
            .split(" ")[0]
            .substring(0, 5)
        : "",
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      setIsEditOpen(false);
      setSuccessMsg(
        "Data konten kamu telah berhasil diperbarui ke dalam sistem SanggaluriMS",
      );
      fetchData();
    } catch (error) {
    ...
    try {
      ...
      const dateTime =
        addForm.date && addForm.time
          ? new Date(`${addForm.date}T${addForm.time}`)
          : null;
      ...
      setIsAddOpen(false);
      setSuccessMsg(
        "Data konten kamu telah berhasil ditambahkan ke dalam sistem SanggaluriMS",
      );
      fetchData();
    } catch (error) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-700">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {columns.map((column, idx) => (
            <div
              key={idx}
              className="bg-white rounded-[2rem] p-6 shadow-sm border border-gray-100 flex flex-col h-fit"
            >
              <div className="flex items-center justify-between mb-8 px-2">
                <h3 className="font-bold text-gray-400 text-sm">
                  {column.title}
                </h3>
                <span className="w-8 h-8 rounded-full bg-gray-50 border border-gray-100 flex items-center justify-center text-xs font-bold text-gray-400">
                  {column.items.length}
                </span>
              </div>
              ...
              <div className="space-y-4">
                {column.items.map((item) => (
                  ...
                  >
                    <h4 className="font-bold text-[#1e293b] text-sm group-hover:text-[#10b981] transition-colors">
                      {item.title}
                    </h4>
                    ...
                    <div className="mt-4 flex items-center justify-between">
                      ...
                      <span className="text-[9px] font-bold text-[#6366f1] uppercase tracking-tighter">
                        {item.format}
                      </span>
                    </div>
                  </div>
        ...
        <div className="p-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setEditForm({ ...editForm, pillar: e.target.value })
                  }
                >
                  {pillarOptions.map((p) => (
          ...
          <div className="flex items-center justify-between mt-8 pt-6 border-t border-gray-100">
            <button
              ...
              className="px-6 py-2.5 border border-[#ef4444]/20 text-[#ef4444] rounded-xl text-sm font-bold hover:bg-[#fef2f2] active:scale-95 transition-all flex items-center gap-2"
            >
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setAddForm({ ...addForm, pillar: e.target.value })
                  }
                >
                  {pillarOptions.map((p) => (
                    <option key={p} value={p}>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setAddForm({ ...addForm, format: e.target.value })
                  }
                >
                  {formatOptions.map((f) => (
                    <option key={f} value={f}>
      ...
      <ConfirmDialog
        ...
        title="Hapus Konten?"
        description={`Apakah kamu yakin ingin menghapus konten "${selectedItem?.title}"?`}
        subDescription="Konten yang dihapus akan hilang dari perencanaan dan tidak bisa dikembalikan."
